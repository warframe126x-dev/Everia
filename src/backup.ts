...
const backupMessages: Record<BackupFailureCode, StringKey> = {
  invalid: "backup.invalid",
  unsupported: "backup.unsupported",
  integrity: "backup.integrity",
  "destination-unavailable": "backup.destinationUnavailable",
  "backup-failed": "backup.failure",
  "restore-failed": "backup.restoreFailed",
  "recovery-failed": "backup.recoveryFailed",
};
export function backupMessageKey(error: unknown): StringKey {
  return error instanceof BackupFailure
    ? backupMessages[error.code]
    : "backup.failure";
}
...
function assert(
  condition: unknown,
  message: string,
  code: BackupFailureCode = "invalid",
): asserts condition {
  if (!condition)
    throw new BackupFailure(code, `Invalid Everia backup: ${message}`);
}
...
function fromBase64(value: string, length: number): Uint8Array {
  assert(
    typeof value === "string" &&
      value.length <= Math.ceil(length / 3) * 4 + 4 &&
  ...
  assert(
    binary.length === length &&
      toBase64(Uint8Array.from(binary, (c) => c.charCodeAt(0))) === value,
    "asset length or encoding",
  );
...
function validId(store: string, id: string) {
  return (
    ...
    id.length <= 128 &&
    new RegExp(
      `^local-${store === "covers" ? "cover" : "wallpaper"}:[A-Za-z0-9_-]+$`,
    ).test(id)
  );
}
function rawState(): RawState {
...
async function entries(assets: StoredAsset[]): Promise<AssetEntry[]> {
  ...
  for (const asset of assets.sort((a, b) =>
    `${a.store}/${a.id}`.localeCompare(`${b.store}/${b.id}`),
  )) {
    assert(
      validId(asset.store, asset.id) && !seen.has(`${asset.store}/${asset.id}`),
      "asset identifier",
    );
    seen.add(`${asset.store}/${asset.id}`);
    assert(
      IMAGE_TYPES.includes(asset.blob.type) &&
        asset.blob.size > 0 &&
        asset.blob.size <= (asset.store === "covers" ? 10 : 30) * 1024 * 1024,
      "asset type or size",
    );
...
function referencedAssets(data: Data, assets: AssetEntry[]) {
  ...
  for (const item of data.items)
    if (item.coverUrl?.startsWith("local-cover:"))
      assert(
        ids.has(`covers/${item.coverUrl}`),
        `missing cover for ${item.id}`,
      );
...
export async function createBackup(
  ...
  assert(
    JSON.stringify(rawState()) === JSON.stringify(before),
    "live library changed during snapshot; retry",
  );
  ...
  const manifest: Document["manifest"] = {
    ...
    assets: assets.map(({ store, id, bytes, sha256 }) => ({
      store,
      id,
      bytes,
      sha256,
...
export async function parseBackup(file: string): Promise<{
  data: Data;
  assets: StoredAsset[];
  fingerprint: string;
  schema: 1 | 2;
  ...
  assert(
    typeof file === "string" &&
      new TextEncoder().encode(file).length <= MAX_FILE,
    "file too large",
  );
  let doc: Document;
  try {
    ...
    throw new BackupFailure(
      "invalid",
      "Invalid Everia backup: truncated or malformed JSON",
    );
  }
  assert(
    doc &&
      typeof doc === "object" &&
      Object.keys(doc).sort().join() === "assets,data,manifest",
    "document shape",
  );
  const { manifest, data, assets } = doc;
  assert(manifest && manifest.format === "EveriaBackup", "backup format");
  assert(
  ...
  assert(
    data &&
      Object.keys(data).sort().join() === "items,settings" &&
      Array.isArray(data.items) &&
      data.items.length <= MAX_RECORDS &&
  ...
  assert(
    manifest.recordCount === data.items.length &&
  ...
  assert(
    typeof manifest.payloadSha256 === "string" &&
      manifest.payloadSha256 ===
        (await hashText(JSON.stringify({ data, assets }))),
    "payload integrity",
    "integrity",
  );
  assert(
    JSON.stringify(validateItems(data.items)) === JSON.stringify(data.items),
    "invalid library records",
  );
  ...
  assert(
    theme &&
      typeof theme === "object" &&
      [theme.accent, theme.background, theme.text].every(
  ...
  assert(
    sorts &&
      typeof sorts === "object" &&
      !Array.isArray(sorts) &&
      Object.values(sorts).every((v) =>
        ["title", "releaseDate", "rating", "dateAdded"].includes(v),
      ),
    "sorts",
  );
  assert(
    views &&
      typeof views === "object" &&
      !Array.isArray(views) &&
      Object.values(views).every((v) => ["grid", "list"].includes(v)),
    "views",
  );
  let total = 0;
  const seen = new Set<string>();
  ...
  for (let i = 0; i < assets.length; i++) {
    ...
    assert(
      asset &&
        ["covers", "wallpapers"].includes(asset.store) &&
        validId(asset.store, asset.id) &&
        !seen.has(`${asset.store}/${asset.id}`) &&
        IMAGE_TYPES.includes(asset.mime) &&
    ...
    assert(
      asset.sha256 === (await hash(bytes)) &&
        JSON.stringify(manifest.assets[i]) ===
          JSON.stringify({
            store: asset.store,
            id: asset.id,
            bytes: asset.bytes,
    ...
    if (typeof createImageBitmap === "function") {
      try {
        ...
        throw new BackupFailure(
          "invalid",
          "Invalid Everia backup: corrupt image",
        );
      }
    }
...
export async function recoverPendingRestore(): Promise<void> {
  ...
  const legacy =
    ...
    Object.keys(pending).sort().join() === "backup,raw" &&
    pending.raw?.length === 4;
  const current =
    pending.version === 2 &&
...
export async function restoreBackup(file: string): Promise<void> {
  ...
  try {
    ...
    const readBackFile = await createBackup(__APP_VERSION__);
    const readBack = await parseBackup(readBackFile);
    const dataReadBack = {
      ...readBack.data,
    ...
    try {
      ...
      throw new BackupFailure("recovery-failed", String(recoveryError)); // Journal remains; bootstrap must block opening the library.
    }
    throw error instanceof BackupFailure
      ? error
      : new BackupFailure("restore-failed", String(error));
  }
}
export async function backUpNow(): Promise<string> {
  ...
  const file = await createBackup(__APP_VERSION__);
  try {
    return await api.write(file);
  } catch (error) {
    const code = (error as { code?: string })?.code;
    throw new BackupFailure(
      code === "destination-unavailable" ? code : "backup-failed",
      String(error),
    );
  }
}
